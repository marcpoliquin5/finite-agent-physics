...
| M15 | Partial | `replanning.py` records digest-bound residual decisions for slowdown, failure, provider capacity, and envelope events; `adaptive_runtime.py` applies provider-reset/capacity and budget-cut events after settled work. | The general replanner is not yet wired as the mutation controller for every active executor event, and first-class settlement-trigger coverage is incomplete. |
...
| M25 | Pass (local) | The SQLite effect broker uses a transactional outbox, idempotency, crash-ambiguity recovery, and an idempotent simulated adapter; crash/duplicate/restart tests yield one physical apply. | Every production target needs equivalent target-side evidence. |
...
| M31 | Pass (local) | `control_api.py` and `control_service.py` expose bearer-protected submit, status, inspect, cancel, exact-scope approve, reference-workflow, and cursor-resumable SSE routes over durable state. Black-box ASGI tests cover strict parsing, auth, CORS, restart, and no direct client database access. | No hosted, OIDC, tenant-RBAC, rate-limit, or HA claim. |
| M32 | Partial | The console renders the sealed v5 candidate evidence, DAG/Gantt/critical path, logical and physical pressure, protected budget, effects, replay, and live/local mode boundaries; its locked build/render tests verify the artifact. | The Sites URL is owner-only, and a complete judge-accessible console-to-public-API path has not been verified. |
...
| M36 | Partial | StormShift runs a typed fictional DAG, revalidates restart outputs, stops at a proposed effect, and gates completion on bounded citation, freshness, authority-taint, bilingual controlled-fact, URL, and static accessibility checks. Its registered deterministic adversarial corpus is refused. | No general entailment/translation-quality proof, rendered browser/assistive-technology audit, live Granite synthesis, or real incident/publication claim. |
...
| R05 | Partial | Bounded controlled-fact citation, number/unit, bilingual equality, freshness, URL, authority separation, and static accessibility checks reject the registered deterministic adversarial corpus. | No general entailment, translation-quality proof, browser-rendered accessibility audit, or source authentication. |
...
| R08 | Partial / External-blocked | The local console, sealed replay, REST/SSE service, candidate verifier, accessibility-oriented UI, and a passing local-suite snapshot exist. | The Sites URL is owner-only; no signed-out judge path, public API, fresh-clone timing, screen-reader audit, or public release/video verification exists. |
